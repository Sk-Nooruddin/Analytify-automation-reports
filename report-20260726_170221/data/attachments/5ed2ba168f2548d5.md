...
Call log:
  - waiting for locator('span').filter({ hasText: 'Caution' }).first()
    - locator resolved to <span draggable="true" data-group="basic" data-key="textarea" data-type="textarea" dndeffectallowed="move" id="cbp_editor_Caution" _ngcontent-ng-c680531232="" class="d-flex btn btn-primary btn-sm btn-block formcomponent drag-copy ng-star-inserted">…</span>
...
```ts
  ...
  10 | //await page.pause();
  11 | page.getByText('Control', { exact: true }).nth(4).click();
  12 | 
  13 | const source = await page.locator('#cbp_editor_Warning:visible')
  14 | const target = await page.getByRole('textbox', { name: 'Section HEADER' })
  15 | await source.dragTo(target);
  16 | const source1 = await page.locator('span').filter({ hasText: 'Caution' }).first()
  17 | const target1 = await page.getByRole('textbox', { name: 'Section HEADER' })
> 18 | await source1.dragTo(target1);
     |               ^ TimeoutError: locator.dragTo: Timeout 30000ms exceeded.
  19 | const source2 = page.locator('span').filter({ hasText: 'Alara' }).first()
  20 | const target2 = await page.getByRole('textbox', { name: 'Section HEADER' })
  21 | await source2.dragTo(target2);
  22 | const source3 = page.locator('span').filter({ hasText: 'Label' }).first()
  23 | const target3 = await page.getByRole('textbox', { name: 'Section HEADER' })
  24 | await source3.dragTo(target3);
  25 | });
```